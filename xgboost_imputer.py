...
class XGBoostImputer:
    ...
    def impute(self):
        """
        Imputes missing values using XGBoost (via IterativeImputer) across specified columns.
        ...
        for col in self.cols:
            if col not in numerical_cols:
                raise ValueError(f"Column '{col}' is not numeric after encoding. Cannot proceed with XGBoost imputation.")
        ...
        # Impute using IterativeImputer with XGBRegressor
        # n_jobs=-1 uses all available cores
        xgb_estimator = XGBRegressor(n_jobs=-1, random_state=self.random_state)
        imputer = IterativeImputer(estimator=xgb_estimator, max_iter=self.max_iter, random_state=self.random_state)
